...
def get_class_distribution(x_y_map: dict) -> tuple[np.float64, np.float64, np.float64, np.float64]:
    """
    Function to calculate the average number of pixels per class in all images from the x_y_map.

    The function effectivley only considers the background,, ILM - OPL, ONL, BMEIS - OP-RPE layers. 
    
    Arguments:
    x_y_map (dict): Dict containing paths to input images as keys and paths to target segmentation 
        images as values.
    
    Returns: 
        tuple[np.float64, np.float64, np.float64, np.float64]: The average number of pixels per image
    ...
    for i in x_y_map:
        ...
        replace = {2: 1, 3: 1, 4: 1, 5: 1, 6: 2, 7: 3, 8: 3, 9: 3, 10: 3, 11: 3}
        for key in replace:
