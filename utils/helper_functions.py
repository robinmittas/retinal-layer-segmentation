...
def transform_dcm_to_npy(paths: list):
    """
    Function to transform DICOM files containing OCT-Scans of shape (z, x, y) into z.

    .npy files of shape (x, y).
...
def transform_segmentation_to_npy(paths_y: list):
    """
    Function to transform .xml files containing segmentations obtained from OCTExplorer.

    of shape (z, x, y) into z .npy files of shape (x, y).
        ...
        for idx_label, layer in enumerate(surface):  # idx corresponds to class label
            ...
            for idx_z, b_scan in enumerate(z_enu):
                for idx_x, idx_y in enumerate(b_scan.find_all("y")):
                    ...
                    segmentation[
                        idx_z, int(idx_y_previous[idx_z, idx_x]) : idx_y_val, idx_x
                    ] = idx_label
...
def get_meta(meta_info, key):
    instance_name = [
        el.keyword for el in meta_info if key.lower() in str(el.keyword).lower()
    ]
...
def get_all_dcm_meta(dc_paths):
    ...
    for dc_path in tqdm(dc_paths):
        ...
        row_dict = {
            ...
            if el.keyword != "" and el.keyword != "PixelData"
        }
        ...
        flag_pydicom_ok, flag_sitk_ok = try_open_dcm(
            pydicom_reader, sitk_image_reader, dc_path
        )
...
def rename_npy_files(x_y_map: dict, sep: str = "/", rename_file=False):
    """
    Helper function to rename files from dictionary which contains x_path as keys and.

    y_path as values.
    """
    # define names all by file name (which often looks like: surname_familyname_thirdname)
    names_all = list(
        set(["_".join(i.split(sep)[-1].split("_")[0:4]) for i in x_y_map.keys()])
    )
...
if __name__ == "__main__":
    directory = (
        "C:\\Users\\robin\\Downloads\\gesunde_OCT_verbessert\\gesunde OCT verbessert"
    )
    x_path = []
    y_path = []
    ...
    for file in sorted(os.listdir(directory)):
        ...
        if (
            ...
            filename.endswith(".xml") and "Final" in filename and "Surfaces" in filename
        ):
