...
class Map:
    ...
    def depth_grid(self, depth_grid_dim=(512, 512)):
        ...
        for i in range(0, len(self.segmentations)):
            ...
            try:
                ...
                print(e, "COULD NOT CALCULATE GRID")
        ...
        if x_cord == "iterable":
            grid_pd_int = pd.DataFrame(grid).interpolate(limit_direction="both", axis=1)
        ...
        if y_cord == "iterable":
            grid_pd_int = pd.DataFrame(grid).interpolate(limit_direction="both", axis=0)
            # set all areas outside of measurements to 0
